...
void commandHandler(File_input *input, char *symbol)
{
    ...
    if (strncmp(input->line.content, ".data", sizeof(".data") - 1) == 0)
    {
        /*.data handling */
        /* Will occur only if the line starts with "label:" */
        if (symbol)
    ...
    if (strncmp(input->line.content, ".entry", sizeof(".entry") - 1) == 0)
    { /*.entry handling*/
...
void dataHandler(File_input *input)
{
    ...
    while ((*input).line.content[0] != '\n')
    {
        ...
        { /* negative value validation, flag will raised if needed */
            sign = -1;
            (input->line.content)++;
        }
        /* digit validation */
        ...
        spaceTrimmer(&((*input).line.content)); /* trim spaces */
        if (input->line.content[0] == ',')
        { /* validate if we excpecting more */
            moreFlag = 1;
            (input->line.content)++;
        }
        else /* if not */
